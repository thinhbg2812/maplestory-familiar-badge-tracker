...
import { badges, totalFamiliars } from '../data/badges'
...
export function useBadgeTracker() {
  ...
  const badgeProgress: ComputedRef<Record<string, BadgeProgress>> = computed(() => {
  ...
  /** Overall progress */
  const overallProgress: ComputedRef<OverallProgress> = computed(() => {
  ...
  const completedBadges: ComputedRef<number> = computed(() => {
  ...
  const nearCompleteBadges: ComputedRef<Badge[]> = computed(() => {
    return [...badges]
      .filter((b) => {
  ...
  function filterBadges(search = '', category = 'all'): Badge[] {
