...
export function useBadgeTracker() {
    const { isCollected, setCollected, toggleCollected, bulkSetCollected, } = useStorage();
    ...
    const completedBadges = computed(() => {
        return badges.filter((b) => badgeProgress.value[b.id]?.complete).length;
    });
    ...
    function filterBadges(search = '', category = 'all') {
        return badges.filter((b) => {
            ...
            const matchesSearch = !search ||
                ...
                b.familiars.some((f) => f.name.toLowerCase().includes(search.toLowerCase()));
