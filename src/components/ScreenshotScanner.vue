<template>
  <div class="scanner animate-fade-in">
    ...
    >
      ...
      <div v-else>
        ...
        <p
          v-if="cvError"
          class="text-sm"
          style="color: var(--c-danger); margin-top: 8px"
        >
          {{ cvError }}
        </p>
      </div>
    ...
    <div v-if="screenshotSrc && cvReady" class="scan-controls">
      ...
      >
        ⚠️ No reference images available. Upload icons in the References tab or
        use a badge with built-in icons.
      </div>
      ...
      <div v-if="scanResults.length > 0" class="results-list">
        ...
        >
          <div class="result-icon">
            <img
              v-if="
                getReferenceImage(result.familiarId) ||
                getBundledIcon(result.familiarId)
              "
              :src="
                getReferenceImage(result.familiarId) ||
                getBundledIcon(result.familiarId)
          ...
          <div class="result-info">
            <div class="result-name">
              {{ getFamiliarName(result.familiarId) }}
            </div>
            <div class="result-meta text-xs text-muted">
              {{ result.matches.length }} match(es) · Best:
              {{ result.matches[0].confidence }} · Scale:
              {{ result.matches[0].scale }}x
            </div>
          </div>
          <div class="result-status">
            <span v-if="isCollected(result.familiarId)" class="tag tag-success"
              >Collected</span
            >
...
onMounted(() => {
  if (!cvReady.value && !cvLoading.value) initCV();
});
onActivated(() => {
  if (!cvReady.value && !cvLoading.value) initCV();
});
