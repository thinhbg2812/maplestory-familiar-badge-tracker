  ...
  <div class="settings animate-fade-in">
    ...
    <section class="setting-section card" style="padding: 20px">
      ...
      <div
        ...
        :style="{
          color: importSuccess ? 'var(--c-success)' : 'var(--c-danger)',
        }"
    ...
    <section class="setting-section card" style="padding: 20px">
      ...
      >
        <p>
          <strong>Maple Badge Tracker</strong> helps you track your MapleStory
          Familiar Card collection.
        </p>
        <p>• Upload reference images of familiar card icons</p>
        <p>• Paste in-game screenshots to auto-detect collected cards</p>
        <p>
          • Uses <strong>OpenCV.js</strong> template matching with multi-scale
          support
        </p>
        <p>
...
const { getSettings, updateSettings, exportData, importData, clearAll } =
  useStorage();

const settings: Settings = reactive(getSettings());
const importMsg = ref("");
const importSuccess = ref(false);

function updateThreshold(e: Event): void {
...
function handleImport(e: Event): void {
  ...
  reader.onload = (ev) => {
    const success = importData((ev.target as FileReader).result as string);
